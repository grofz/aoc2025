...
!
! CONSTRUCTION FORMULAS FOR HEAPS
! d-ary heap dimension: d = 2, 3, ...
! element index: i = 1, 2, ..., n
! (leftmost child index)  = i*d-(d-2)
! (rightmost child index) = i*d+1
! (parent index)          = floor[ (i+(d-2))/d ] 
! (last non-leaf node)    = floor[ (n+(d-2))/d ] 
!
! TODO Extend code for heaps of d > 2
  ...
  module pqueue_mod
    ...
    abstract interface
      ...
      integer(i8) function priority(this)
        import element_t, i8
